...
import com.shaddaystore_mobileapp_client.adapter.ProductoCarritoAdapter;
        ...
        btnFinalizarCompra.setOnClickListener(v -> {
            ...
            if (idC != 0) {
                if (Carrito.getDetallePedidos().isEmpty()) {
                    toastIncorrecto("¡Oops! La bolsa de compra está vacia.");
                } else {
                    toastCorrecto("Procesando su pedido, por favor espere...");
                    registrarPedido(idC);
                }
            } else {
                toastIncorrecto("No ha iniciado sesión, se le enviará al login.");
    ...
    private void registrarPedido(int idC) {
        ...
        this.pedidoViewModel.guardarPedido(dto).observe(this, response -> {
            if (response.getRpta() == 1) {
                toastCorrecto("¡Su pedido ha sido registrado con éxito!");
                Carrito.limpiar();
                finish();
                overridePendingTransition(R.anim.left_in, R.anim.left_out);
            } else {
                toastIncorrecto("Lo sentimos, no se ha registrado el pedido.");
            }
        });
